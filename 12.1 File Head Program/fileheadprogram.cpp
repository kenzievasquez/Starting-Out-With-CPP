...
void searchString(std::fstream& file, std::string searchWord);

int main(){
    std::fstream file;
    std::string filename,
                searchWord="grasshoppers";
...
/*/////////////////////////////////////////////////////////////////////////////
    Name:    searchString
    Purpose: Searches a string within a file
    Inputs:  std::fstream& file
             std::string str
    Output: 
*//////////////////////////////////////////////////////////////////////////////
void searchString(std::fstream& file, std::string searchWord){
    ...
    std::vector<std::string> occurence,
                             words;
    std::string word;
    unsigned short searchWordCount=0;

    unsigned short index;

    // Read in the file line by line, and add LINE (where searchWord occurs in) 
    // to a vector while keeping COUNT of every occurence
    while(std::getline(file, line)){
        line.find(searchWord);
        occurence.push_back(line);

        // Search the searchWord within each vector
        for(){
